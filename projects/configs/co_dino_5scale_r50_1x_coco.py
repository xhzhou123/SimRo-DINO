...
model = dict(
    ...
    neck=dict(
        type='ChannelMapper',
        # Map the feature maps of input channel at each scale to the specified output channel numbers.
    ...
    query_head=dict(
        ...
        loss_con=dict(type='CosineSimilarityLoss', loss_weight=1.0)),
    roi_head=[dict(
        ...
        dict(
            rpn=dict(
                ...
                sampler=dict(
                    type='RandomSampler',
                    num=256,
                    pos_fraction=0.5,       # Balance pos-neg-sample pairs
                    neg_pos_ub=-1,  # Set up neg-sample's up boundary
                    add_gt_as_proposals=False), # Add gound truth as proposals to guide training in early epoch
                allowed_border=-1,
                pos_weight=-1,
                debug=False),
            rpn_proposal=dict(
