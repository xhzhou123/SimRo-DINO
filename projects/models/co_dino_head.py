...
class CoDINOHead(CoDeformDETRHead):
    ...
    def __init__(self,
        ...
        self.encoder = nn.Sequential(nn.Linear(self.prev_dim, self.prev_dim, bias=False), # 512
                                        nn.BatchNorm1d(self.prev_dim),
                                        nn.ReLU(inplace=True), # first layer 512
                                        nn.Linear(self.prev_dim, self.prev_dim, bias=False), # 512
                                        nn.BatchNorm1d(self.prev_dim),
                                        nn.ReLU(inplace=True), # second layer 512
                                        nn.Linear(self.prev_dim, self.dim), # 1024
                                        nn.BatchNorm1d(self.dim, affine=False)) # 1024
        self.encoder[6].bias.requires_grad = False
        self.predictor = nn.Sequential(nn.Linear(self.dim, self.prev_dim, bias=False),
                                        nn.BatchNorm1d(self.prev_dim),
                                        nn.ReLU(inplace=True), # hidden layer
                                        nn.Linear(self.prev_dim, self.dim)) # output layer
    ...
    def forward_train(self,
        ...
        # z1 = torch.concat((self.avgpool(x[0]),self.avgpool(x[4])),1)
        # z2 = torch.concat((self.avgpool(sub_x[0]),self.avgpool(sub_x[4])),1)
        z1 = self.avgpool(x[0])
        z2 = self.avgpool(sub_x[0])
        ...
        contr_ls=[z1.detach(),z2.detach(),p1,p2]
        z1 = z1.detach()
        z2 = z2.detach()
        p1 = p1.detach()
        p2 = p2.detach()
        ...
        if gt_labels is None:
            loss_inputs = outs + (gt_bboxes, img_metas, dn_meta, contr_ls)
        else:
            loss_inputs = outs + (gt_bboxes, gt_labels, img_metas, dn_meta, contr_ls)
    ...
    def loss(self,
        ...
        loss_contr = ((1-self.loss_con(contr_ls[2],contr_ls[1])).mean()+(1-self.loss_con(contr_ls[3],contr_ls[0])).mean())*0.5
        contr_ls[2] = contr_ls[2].detach()
        contr_ls[3] = contr_ls[3].detach()
        loss_dict['loss_contr'] = loss_contr
