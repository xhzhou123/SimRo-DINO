...
class DinoTransformerDecoder(DeformableDetrTransformerDecoder):
    ...
    def _init_layers(self):
        self.ref_point_head = build_MLP(self.embed_dims * 2, self.embed_dims,
                                        self.embed_dims, 2)
        self.ref_point_head[0].weight.requires_grad = False
        self.ref_point_head[0].bias.requires_grad = False
        self.ref_point_head[2].weight.requires_grad = False
        self.ref_point_head[2].bias.requires_grad = False
        self.ref_rotate = build_MLP(self.embed_dims, self.embed_dims//2,
                                        self.embed_dims//2, 2)
    ...
    @staticmethod
    def gen_rotateembed_for_position(pos_tensor, pos_feat):
        # n_query, bs, _ = pos_tensor.size()
        # sineembed_tensor = torch.zeros(n_query, bs, 256)
        B,N,_=pos_tensor.shape
        dim_t = torch.arange(
            pos_feat//2, dtype=torch.float32, device=pos_tensor.device)
        dim_t = 10000**(2 * dim_t / pos_feat)
        x_embed = pos_tensor[:, :, 0]
        y_embed = pos_tensor[:, :, 1]
        x_freqs = torch.outer(x_embed.flatten(), 1.0/dim_t).float().reshape(B,N,-1)
        y_freqs = torch.outer(y_embed.flatten(), 1.0/dim_t).float().reshape(B,N,-1)
        if pos_tensor.size(-1) == 2:
            pos = torch.cat((y_freqs, x_freqs), dim=2)
        elif pos_tensor.size(-1) == 4:
            w_embed = pos_tensor[:, :, 2]
            w_freqs = torch.outer(w_embed.flatten(), 1.0/dim_t).float().reshape(B,N,-1)

            h_embed = pos_tensor[:, :, 3]
            h_freqs = torch.outer(h_embed.flatten(), 1.0/dim_t).float().reshape(B,N,-1)

            pos = torch.cat((y_freqs, x_freqs, w_freqs, h_freqs), dim=2)
        else:
            raise ValueError('Unknown pos_tensor shape(-1):{}'.format(
                pos_tensor.size(-1)))
        return pos
    ...
    def forward(self,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            query_sine_embed = self.gen_rotateembed_for_position(
                reference_points_input[:, :, 0, :], self.embed_dims//2)
            query_pos = self.ref_rotate(query_sine_embed)
...
class CoDinoTransformer(CoDeformableDetrTransformer):
    ...
    def init_layers(self):
        ...
        self.level_embeds_rotate = nn.Parameter(
            torch.Tensor(self.num_feature_levels, self.embed_dims//2))
    ...
    def forward(self,
        ...
        lvl_pos_embed_flatten = []
        spatial_shapes = []
        for lvl, (feat, mask, pos_embed) in enumerate(      # level 0: [1, 256, 200, 332] [[1.,1.,...,1.,1.],[2.,2.,...,2.,2.]...]
                zip(mlvl_feats, mlvl_masks, mlvl_pos_embeds)):
            bs, c, h, w = feat.shape
            spatial_shape = (h, w)
            spatial_shapes.append(spatial_shape)
            feat = feat.flatten(2).transpose(1, 2)
            mask = mask.flatten(1)
            pos_embed = pos_embed.flatten(2).transpose(1, 2)    # level 0: [1, 66400, 256] [1.,1.,...,1.,1.,2.,2.,...,2.,2....]
            lvl_pos_embed = pos_embed + self.level_embeds_rotate[lvl].view(1, 1, -1)   # level 0: [1, 66400, 256] [0.6757,0.6757,0.6757,...,199.6757, 199.6757,199.6757]
        ...
        feat_flatten = torch.cat(feat_flatten, 1)
        mask_flatten = torch.cat(mask_flatten, 1)
        lvl_pos_embed_flatten = torch.cat(lvl_pos_embed_flatten, 1)     # [1, 66400+16600+4250+1050+273, 256] [0.6757,0.6757,0.6757,...,13.3575,13.3575,13.3575]
        spatial_shapes = torch.as_tensor(
            spatial_shapes, dtype=torch.long, device=feat_flatten.device)
        level_start_index = torch.cat((spatial_shapes.new_zeros(
            (1, )), spatial_shapes.prod(1).cumsum(0)[:-1]))
